...
class user():
    ...
    def spider(self):
        ...
        # 处理视频信息
        videoCode= soup.find_all('div', attrs={'class': 'work-card'})
        for i in range(len(videoCode)):
            link = videoCode[i].find_all('img')[0].get('src')
            ...
            self.videoList.append([link, title, publishTime, playNum, likeNum, commentNum])
