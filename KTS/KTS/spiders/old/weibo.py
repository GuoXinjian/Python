...
class User_Weibo():#一个实例=一个账号
    ...
    @staticmethod
    def getUserInfo(userid,proxy=None,cookies=None,headers=None):
...
class Superindex_Weibo():
    @staticmethod
    def getSuperindexInfo(keyword,proxy=None,cookies=None,headers=None):
        if keyword[:6]=='100808':
            keyword=keyword[6:]
    
        print('查询超级话题数据......')
        apiurl='https://m.weibo.cn/api/container/getIndex?containerid=100808'+keyword
        response=requests.get(apiurl,proxies=proxy,cookies=cookies,headers=headers,timeout=3)
        if response.status_code==403:
            print('403：服务器拒绝访问')
        elif response.status_code==503:
            ...
            valid=res['ok']
            if valid==0:
                print(keyword,':未查到超级话题')
            else:
                print('成功')
                data=res['data']['pageInfo']
        #pattern=re.compile(r'^')
        desc_more=data['desc_more'][0]
        data={
            'containerid':data['containerid'],
            'page_type_name':data['page_type_name'],
            'title_top':data['title_top'],
            'title_icon':data['title_icon'],
            'nick':data['nick'],
            'page_url':data['page_url'],
            'detail_desc':data['detail_desc'],
            'portrait_sub_text':data['portrait_sub_text'],
            'desc':data['desc'],
            'read':toInt(re.search(r'阅读([1-9][0-9]*)+(.[0-9]*)?\S',desc_more).group()),
            'statuses':toInt(re.search(r'帖子([1-9][0-9]*)+(.[0-9]*)?\S',desc_more).group()),
            'fans':toInt(re.search(r'粉丝([1-9][0-9]*)+(.[0-9]*)?\S',desc_more).group()),
        }
        return data
...
if __name__ == "__main__":
    ...
    '''
    h=Superindex_Weibo.getSuperindexInfo('1008081a29877c30ad981d54a7d4cb5299e154')
    print(h)
    '''
